...
class SmashMode(Enum):
    ...
    LUDICROUS = "ludicrous"    # Fire ALL, fastest wins
...
class SmashResult:
    ...
    elapsed: float
    success: bool
    mode: SmashMode = SmashMode.LUDICROUS
    cost: float = 0.0
    tokens: int = 0
...
class SmartRouter:
    ...
    def _setup_default_workers(self):
        ...
        defaults = [
            WorkerConfig("openai", "gpt-4o-mini", "OPENAI_API_KEY",
                        "https://api.openai.com/v1/chat/completions",
                        rate_limit=500, cost_per_1k=0.00015, typical_latency=8.0),
            WorkerConfig("azure_openai", "gpt-4o", "AZURE_OPENAI_API_KEY",
                        os.environ.get("AZURE_OPENAI_ENDPOINT", ""),
                        rate_limit=500, cost_per_1k=0.0002, typical_latency=8.0),
            WorkerConfig("groq", "llama-3.3-70b-versatile", "GROQ_API_KEY",
                        "https://api.groq.com/openai/v1/chat/completions",
                        rate_limit=30, is_free=True, typical_latency=1.5),
            WorkerConfig("gemini", "gemini-2.0-flash", "GOOGLE_API_KEY",
                        "https://generativelanguage.googleapis.com/v1beta/models",
                        rate_limit=60, is_free=True, typical_latency=0.5),
            WorkerConfig("local", "llama3.1:8b", "",
                        "http://localhost:11434/api/generate",
                        rate_limit=9999, is_free=True, is_local=True, typical_latency=5.0),
            WorkerConfig("together", "meta-llama/Llama-3.3-70B-Instruct-Turbo", "TOGETHER_API_KEY",
                        "https://api.together.xyz/v1/chat/completions",
                        rate_limit=60, is_free=True, typical_latency=2.0),
            WorkerConfig("deepseek", "deepseek-chat", "DEEPSEEK_API_KEY",
                        "https://api.deepseek.com/v1/chat/completions",
                        rate_limit=60, is_free=True, typical_latency=2.0),
            WorkerConfig("openrouter", "auto", "OPENROUTER_API_KEY",
                        "https://openrouter.ai/api/v1/chat/completions",
                        rate_limit=20, is_free=True, typical_latency=3.0),
        ]
        for w in defaults:
    ...
    async def route(
        ...
        if mode == SmashMode.LUDICROUS:
            from .coordinator import ludicrous_smash

            return await ludicrous_smash(prompt, timeout=timeout, workers=workers)
        ...
        if posture is not None:
            ...
            if posture.prefer_free_workers:
                worker_names = sorted(
                    worker_names,
                    key=lambda name: (
                        not self.workers.get(name, WorkerConfig(name, "", "", "")).is_free,
        ...
        if mode == SmashMode.CONSENSUS:
            ...
            consensus_workers = [w for w in worker_names if w in coolest] or worker_names
            result = await ludicrous_smash(
    ...
    def record_result(self, result: SmashResult):
        ...
        stats = self.stats.setdefault(result.provider, 
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "coolest": self.get_coolest_workers(3),
            "free_workers": [n for n, w in self.workers.items() if w.is_free and w.enabled],
        }
