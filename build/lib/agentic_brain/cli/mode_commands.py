...
def mode_list_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"{Colors.DIM}Use 'ab mode info <CODE>' for detailed information{Colors.RESET}\n")
...
def mode_switch_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Perform the switch
        transition = manager.switch(code, announce=not args.quiet if hasattr(args, 'quiet') else True)
        ...
        if transition.success:
            ...
            print(f"  {Colors.DIM}Switch time: {transition.duration_ms:.2f}ms{Colors.RESET}")
            return 0
        else:
...
def mode_current_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"  {Colors.BOLD}Name:{Colors.RESET}        {Colors.GREEN}{mode.name}{Colors.RESET}")
        print(f"  {Colors.BOLD}Code:{Colors.RESET}        {Colors.CYAN}{mode.code}{Colors.RESET}")
        ...
        # Show key settings
        print(f"  {Colors.BOLD}LLM Model:{Colors.RESET}   {mode.config.llm.primary_model}")
        print(f"  {Colors.BOLD}RAG Type:{Colors.RESET}    {mode.config.rag.rag_type.value}")
        print(f"  {Colors.BOLD}Security:{Colors.RESET}    {mode.config.security.level.value}")
        print(f"  {Colors.BOLD}Voice:{Colors.RESET}       {mode.config.voice.primary_voice}")
        print()
...
def mode_wizard_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        categories = {
            ...
            "5": ("POWER", "Ludicrous, Plaid (maximum performance)"),
        }
        ...
        for i, mode in enumerate(category_modes, 1):
            print(f"  [{i}] {Colors.CYAN}{mode.code:5}{Colors.RESET} {mode.name:15} - {mode.description[:45]}")
        print()
        
        try:
            mode_choice = input(f"{Colors.BOLD}Enter choice (1-{len(category_modes)}): {Colors.RESET}").strip()
        except (KeyboardInterrupt, EOFError):
            print("\n")
            print_info("Wizard cancelled")
            return 0
        ...
        try:
            confirm = input(f"Switch to this mode? [Y/n]: ").strip().lower()
        except (KeyboardInterrupt, EOFError):
            print("\n")
            print_info("Wizard cancelled")
            return 0
...
def mode_stats_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"  {Colors.BOLD}Current Mode:{Colors.RESET}      {stats['current_mode'] or 'None'}")
        print(f"  {Colors.BOLD}Total Modes:{Colors.RESET}       {stats['total_modes']}")
        print(f"  {Colors.BOLD}Total Switches:{Colors.RESET}    {stats['total_switches']}")
        print(f"  {Colors.BOLD}Avg Switch Time:{Colors.RESET}   {stats['avg_switch_time_ms']:.2f}ms")
        print(f"  {Colors.BOLD}Min Switch Time:{Colors.RESET}   {stats['min_switch_time_ms']:.2f}ms")
        print(f"  {Colors.BOLD}Max Switch Time:{Colors.RESET}   {stats['max_switch_time_ms']:.2f}ms")
        print()
        ...
        if stats['recent_transitions']:
            ...
            for t in stats['recent_transitions']:
                ...
                print(f"  {status}{Colors.RESET} {t['from'] or 'None'} → {t['to']} ({t['duration_ms']:.2f}ms)")
            print()
