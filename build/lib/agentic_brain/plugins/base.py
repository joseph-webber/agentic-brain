        ...
        if event in self._hooks:
            for handler in self._hooks[event]:
                try:
                    ...
                    logger.error(
                        f"Error in {self.name} hook {event}: {e}", exc_info=True
                    )
...
class PluginManager:
    ...
    def _load_configs(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.configs)} plugin configs from {self.config_path}"
            )
        except Exception as e:
    ...
    def load_plugin(
        self, plugin_class: type[Plugin], config: Optional[PluginConfig] = None
    ) -> Plugin:
        """
                    ...
                    for _name, obj in inspect.getmembers(module):
                        if inspect.isclass(obj) and obj is not Plugin:
                            ...
                            if plugin:
                                ...
                                logger.info(
                                    f"Discovered and loaded plugin: {plugin.name}"
                                )
            except Exception as e:
    ...
    def trigger(self, hook_name: str, *args, **kwargs) -> Optional[Any]:
        """
