...
from .registry import (
    MODE_REGISTRY, CODE_TO_NAME, MODES_BY_CATEGORY,
    get_mode, list_modes, get_mode_count,
)
...
class ModeManager:
    ...
    Example:
        ...
        manager.switch("ludicrous")  # Switch to Ludicrous mode
    ...
    def switch(
        ...
        Args:
            name_or_code: Mode name or short code (e.g., "D", "developer", "ludicrous")
        ...
        try:
            ...
            # Voice announcement
            if announce and self._voice_enabled and new_mode.config.voice.announce_mode_changes:
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "min_switch_time_ms": round(min(self._switch_times), 2) if self._switch_times else 0,
            "max_switch_time_ms": round(max(self._switch_times), 2) if self._switch_times else 0,
            "recent_transitions": [
                {
                    "from": t.from_mode,
                    "to": t.to_mode,
    ...
    def search(self, query: str) -> List[Mode]:
        ...
        for mode in MODE_REGISTRY.values():
            if (
                ...
                query_lower in mode.code.lower() or
                query_lower in mode.description.lower()
            ):
                results.append(mode)
    ...
    def _save_state(self) -> None:
        ...
        try:
            state = {
                "current_mode": self._current_mode.name if self._current_mode else "developer",
                "timestamp": time.time(),
                "total_switches": len(self._transitions),
            }
