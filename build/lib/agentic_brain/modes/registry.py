...
from .base import (
    Mode, ModeConfig, ModeCategory,
    LLMConfig, RAGConfig, SecurityConfig, VoiceConfig,
    ComplianceConfig, ResourceConfig,
    RagType, SecurityLevel, VoicePersonality,
    create_developer_llm, create_enterprise_security,
    create_hipaa_compliance, create_gdpr_compliance,
    create_ludicrous_resources,
)
...
def _build_mode_registry() -> Dict[str, Mode]:
    ...
    modes["ludicrous"] = Mode(
        ...
        config=ModeConfig(
            ...
            resources=create_ludicrous_resources(),
            voice=VoiceConfig(
                personality=VoicePersonality.ENERGETIC,
                primary_voice="Alex",
            ),
    ...
    modes["plaid"] = Mode(
        ...
        code="P",
        category=ModeCategory.POWER,
        description="Beyond Ludicrous. Experimental features, bleeding edge, no safety limits.",
        icon="🌈",
        color="#8B5CF6",
        config=ModeConfig(
            llm=LLMConfig(
...
def list_modes(category: ModeCategory = None) -> List[Mode]:
    """
...
def get_mode_count() -> Dict[str, int]:
    ...
    return {
        cat.value: len(modes) for cat, modes in MODES_BY_CATEGORY.items()
    }
