...
def get_api_key(config: RouterConfig) -> str:
    ...
    """
    api_key = config.together_key or os.environ.get("TOGETHER_API_KEY")
    if not api_key:
        raise ConfigurationError(
            "TOGETHER_API_KEY",
            "valid Together API key from https://www.together.ai/",
            "3c1f6d...",
        )
    return api_key
...
async def chat_together(
    ...
    logger.debug(f"Together chat: model={model}, temperature={temperature}")
    ...
    try:
        status, result = await post_fn(
            url, payload, headers, timeout=config.together_timeout
        )

        if status == 429:
            raise RateLimitError(limit=100, window="minute", retry_after=60)
        ...
        logger.info(
            f"LLM response received: provider={Provider.TOGETHER.value}, tokens={tokens_used}"
        )

        # Track token usage
        track_tokens_fn(tokens_used, Provider.TOGETHER.value)
        ...
        raise LLMProviderError(
            "together", model, Exception(f"Invalid response from Together: {e}")
        )
...
async def stream_together(
    ...
    async with (
        ...
        session.post(
            ...
            timeout=aiohttp.ClientTimeout(total=config.together_timeout),
        ) as response,
    ):
