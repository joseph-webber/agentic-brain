...
@mcp.tool()
def rg_search(pattern: str, path: str = "~/brain", file_type: str = None, 
...
@mcp.tool()
def fd_find(pattern: str = "", path: str = "~/brain", extension: str = None,
            file_type: str = None, max_depth: int = None, max_results: int = 50) -> dict:
    """
...
@mcp.tool()
def eza_list(path: str = ".", long: bool = True, all_files: bool = False,
             tree: bool = False, tree_depth: int = 2, sort: str = "name") -> dict:
    """
...
def tldr_help(command: str) -> dict:
    ...
    return {
        ...
        "command": command,
        "help": result["output"] if result["success"] else result.get("error", "Command not found")
    }
...
@mcp.tool()
def sox_convert(input_file: str, output_file: str, 
...
@mcp.tool()
def sox_trim(input_file: str, output_file: str, start: str, duration: str = None) -> dict:
    """
    Trim/cut audio file.
    ...
    """
    cmd = ["sox", os.path.expanduser(input_file), os.path.expanduser(output_file), 
           "trim", start]
    ...
    if result["success"]:
        return {
            "success": True,
            "output": output_file,
            "message": f"Trimmed from {start}" + (f" for {duration}" if duration else "")
        }
...
def cli_tools_status() -> dict:
    ...
    for name, cmd in tools.items():
        ...
        if result["success"]:
            ...
            status[name] = {"installed": False, "error": result.get("error", "Not found")}
    
    installed = sum(1 for s in status.values() if s["installed"])
    
    return {
        "success": True,
        "installed": f"{installed}/{len(tools)}",
        "tools": status
    }
