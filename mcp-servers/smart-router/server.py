...
def smart_cycle_order(base_order: list[str]) -> list[str]:
    ...
    def score(p: str) -> float:
        ...
        recent_requests = len(
            [t for t in rate_limits.get(p, []) if time.time() - t < 60]
        )
        rate_pressure = (
            recent_requests / max(PROVIDERS[p].rate_limit, 1) if p in PROVIDERS else 0
        )
    ...
    if cycle_index > 0 and len(reordered) > 1:
        ...
        if provider_heat.get(reordered[0], 0) > 2.0:  # If first is warm
            ...
            for i in range(1, len(reordered)):
                if provider_heat.get(reordered[i], 0) < provider_heat.get(
                    reordered[0], 0
                ):
