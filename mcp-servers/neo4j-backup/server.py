...
def _ensure_config():
    ...
    ICLOUD_DIR = Path.home() / "Library/Mobile Documents/com~apple~CloudDocs/brain-backups/neo4j"
...
def backup_status() -> dict:
    ...
    recent_backups = [f for f in files if datetime.fromtimestamp(f.stat().st_mtime) > datetime.now() - timedelta(days=7)]
...
def backup_list(limit: int = 10) -> dict:
    ...
    return {"total": len(files), "showing": min(limit, len(files)), "backups": "\n".join(lines)}
...
def backup_browse(filename: str, query: str = None) -> dict:
    ...
    try:
        ...
        if query:
            nodes = [n for n in data.get("nodes", []) if query.lower() in str(n).lower()][:10]
            return {
                "query": query,
        ...
        return {
            ...
            "relationship_types": dict(sorted(rel_counts.items(), key=lambda x: -x[1])[:20])
        }
    except Exception as e:
        return {"error": str(e)}
...
def backup_restore(filename: str, confirm: bool = False) -> dict:
    ...
    if not confirm:
        return {"error": "RESTORE BLOCKED: Set confirm=true to restore. This will REPLACE all current Neo4j data!"}
    ...
    try:
        ...
        with driver.session() as session:
            ...
            for node in data.get("nodes", []):
                ...
                result = session.run(f"CREATE (n:{labels} $props) RETURN elementId(n)", props=props)
...
def backup_health() -> dict:
    ...
    recent = [f for f in files if datetime.fromtimestamp(f.stat().st_mtime) > datetime.now() - timedelta(days=7)]
...
def backup_metrics() -> dict:
    ...
    return {
        ...
        "output": json.dumps({
            ...
            "smallest_backup": min(files, key=lambda f: f.stat().st_size).name if files else None,
        }, indent=2)
    }
...
def backup_simulate_gfs() -> dict:
    ...
    return {
        "policy": "Keep backups from last 30 days",
        "keep": keep,
        "delete": delete
    }
