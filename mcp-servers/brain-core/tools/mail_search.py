...
def _search_neo4j(query: str, sender: Optional[str], since: datetime, limit: int) -> List[Dict]:
    """
...
def _search_spotlight(query: str, sender: Optional[str], since: datetime, limit: int) -> List[Dict]:
    """
    ...
    try:
        ...
        if query:
            search_terms.append(f'(kMDItemTextContent == "*{query}*" || kMDItemSubject == "*{query}*")')
        ...
        if since:
            ...
            search_terms.append(f'kMDItemContentModificationDate >= $time.iso({date_str})')
        ...
        for filepath in files[:limit]:
            try:
                ...
                mdls = subprocess.run(
                    ['mdls', '-name', 'kMDItemSubject', '-name', 'kMDItemAuthors', filepath],
        ...
        for email in recent:
            try:
                ...
                """, {
                    ...
                    'timestamp': email.get('date', email.get('timestamp', datetime.now().isoformat())),
...
def mail_recent(days: int = 7, limit: int = 20, speak: bool = True) -> Dict[str, Any]:
    ...
    if speak:
        ...
        _speak(f"You have {count} email{'s' if count != 1 else ''} from the last {days} days")
