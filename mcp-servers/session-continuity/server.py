...
def emit_session_event(event_type: str, data: dict):
    ...
    if bus:
        try:
            bus.emit("brain.session.checkpoint", {
...
def _emergency_shutdown_save():
    ...
    try:
        ...
        state = {
            ...
            "context_buffer": _current_session.get("context_buffer", [])[-50:],  # Last 50 items
...
async def auto_checkpoint_loop():
    ...
    while True:
        ...
        try:
            ...
            emit_session_event("auto_checkpoint", {
...
async def session_lifespan(mcp_server: FastMCP):
    ...
    if crash_info:
        ...
        if recovered_state:
            emit_session_event("crash_recovered", {
        ...
        if STATE_FILE.exists():
            try:
                ...
                emit_session_event("previous_session_found", {
    ...
    emit_session_event("session_started", {
    ...
    try:
        ...
        try:
            duration = (datetime.now() - datetime.fromisoformat(_current_session["started_at"])).total_seconds()
            ...
            emit_session_event("session_ended", {
...
def emergency_save_context(reason: str, context_data: Dict = None):
    ...
    try:
        ...
        emit_session_event("emergency_save", {
            "reason": reason,
            "session_id": _current_session.get("id")
        })
...
def recover_from_crash() -> Dict:
    ...
    if EMERGENCY_SAVE.exists():
        try:
            ...
            archive_path = CONTINUITY_DIR / "crashes" / f"emergency_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    if driver:
        try:
            ...
            if context_events or conversations:
                recovery["recovery_quality"] = "full" if len(recovery["sources_checked"]) >= 2 else "partial"
...
def save_state(
    ...
    if not auto_save:  # Don't spam events for auto-saves
        emit_session_event("manual_save", {
            "session_id": session_id,
...
@mcp.tool()
def session_save(description: str = "", blockers: list = None, learnings: list = None) -> dict:
    """
...
def context_log(
    ...
    context_id = f"ctx_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:6]}"
    ...
    try:
        ...
        # Emit to event bus
        emit_session_event("context_logged", {
            "context_id": context_id,
            "event_type": event_type,
            "summary": summary[:100]
        })
...
def context_compact() -> dict:
    ...
    try:
        with driver.session() as session:
            # Count before
            before = session.run("MATCH (c:ContextEvent) RETURN count(c) as count").single()["count"]
            ...
            # Count after
            after = session.run("MATCH (c:ContextEvent) RETURN count(c) as count").single()["count"]
        ...
        emit_session_event("context_compacted", {
            "deleted": deleted,
            "remaining": after
        })
    ...
    try:
        with driver.session() as session:
            ...
            """,
                ...
                assistant_response=assistant_response[:2000] if assistant_response else "",
                topics=topics or [],
                sentiment=sentiment,
                session_id=_current_session.get("id", "unknown")
            )
    ...
    if driver:
        try:
            with driver.session() as session:
                ...
                """, sid=_current_session.get("id", "unknown")).single()["count"]
...
def neo4j_session_stats() -> dict:
    ...
    try:
        with driver.session() as session:
            ...
            # Count each type
            for label in ["Session", "ContextEvent", "ConversationTurn", "Learning", "Checkpoint"]:
            ...
            priority_dist = session.run("""
                ...
                ORDER BY count DESC
            """)
            stats["context_by_priority"] = {r["priority"]: r["count"] for r in priority_dist}
...
def continuity_health() -> dict:
    ...
    return {
        ...
        "connections": {
            ...
            "event_bus": "connected" if EVENT_BUS_AVAILABLE and _event_bus else "not connected"
        }
    }
