    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return {"success": True, "response": result.stdout.strip(), 
                    "model": model, "time": f"{elapsed:.2f}s"}
        save_metrics(metrics)
...
def local_status() -> dict:
    ...
    except: loaded = "Unknown"
    
    avg = metrics["total_time"] / metrics["requests"] if metrics["requests"] > 0 else 0
    rate = metrics["successes"] / metrics["requests"] * 100 if metrics["requests"] > 0 else 0
...
def local_warmup(model: str = "fast") -> dict:
    ...
    return run_ollama(MODELS.get(model, MODELS["fast"]), "warmup", 
...
def local_benchmark() -> dict:
    ...
    for name, model in MODELS.items():
        start = time.time()
        try:
            r = subprocess.run(["ollama", "run", model, "Say: OK"], 
                              capture_output=True, text=True, timeout=120)
            results[name] = {"time": f"{time.time()-start:.2f}s", "ok": r.returncode == 0}
        except: results[name] = {"ok": False, "error": "timeout"}
    return results
