...
TOOLS = [
    ...
    ("apple_say", "Text to speech", {"text": {"type": "string"}, "voice": {"type": "string"}}),
...
async def list_tools() -> List[Tool]:
    return [Tool(name=n, description=d, inputSchema={"type": "object", "properties": p}) for n, d, p in TOOLS]

@server.call_tool()
async def call_tool(name: str, args: Dict[str, Any]) -> List[TextContent]:
    try:
        if name == "apple_hardware":
            r = subprocess.run(["system_profiler", "SPHardwareDataType", "-json"], capture_output=True, text=True)
            ...
            r = subprocess.run(["osascript", "-e", 'tell app "System Events" to get name of every process whose background only is false'], capture_output=True, text=True)
            ...
            if "level" in args:
                subprocess.run(["osascript", "-e", f"set volume output volume {args['level']}"])
                result = {"volume": args["level"]}
            else:
                r = subprocess.run(["osascript", "-e", "output volume of (get volume settings)"], capture_output=True, text=True)
                result = {"volume": int(r.stdout.strip()) if r.stdout.strip().isdigit() else 50}
        elif name == "apple_dark_mode":
            if "enable" in args:
                mode = "true" if args["enable"] else "false"
                subprocess.run(["osascript", "-e", f'tell app "System Events" to tell appearance preferences to set dark mode to {mode}'])
                result = {"dark_mode": args["enable"]}
            else:
                r = subprocess.run(["osascript", "-e", 'tell app "System Events" to tell appearance preferences to get dark mode'], capture_output=True, text=True)
                result = {"dark_mode": r.stdout.strip() == "true"}
        elif name == "apple_say":
            subprocess.run(["say", "-v", args.get("voice", "Samantha"), args.get("text", "")])
            result = {"said": args.get("text", "")[:50]}
        elif name == "apple_spotlight":
            r = subprocess.run(["mdfind", args.get("query", ""), "-limit", "20"], capture_output=True, text=True)
            result = {"files": r.stdout.strip().split("\n")[:20]}
        elif name == "apple_weather":
            r = subprocess.run(["curl", "-s", f"wttr.in/{args.get('location', 'London')}?format=3"], capture_output=True, text=True)
            result = {"weather": r.stdout.strip()}
        elif name == "apple_system_info":
            r = subprocess.run(["system_profiler", "SPSoftwareDataType", "-json"], capture_output=True, text=True)
            result = json.loads(r.stdout) if r.returncode == 0 else {"error": "Failed"}
        else:
