...
# Kick & Bass Demo
try:
    from kick_bass_demo import (
        KickBassDemoEngine, PARAMS, load_style, list_midi_styles
    )
...
# Configuration
VIRUS_BINARY = os.path.expanduser("~/gearmulator/build_cli/source/virusRealtime/virusRealtime")
...
# Category mapping
CATEGORIES = [
    "None", "Lead", "Bass", "Pad", "Decay", "Pluck", "Acid", "Classic",
    "Arpeggiator", "Effects", "Drums", "Percussion", "Input", "Vocoder",
    "Favourite1", "Favourite2", "Favourite3", "Organ", "Piano", "String",
    "FM", "Digital", "Atomizer"
]
...
class VirusPatchServer:
    ...
    def search_patches(self, query: str, category: str = None, limit: int = 20, fuzzy_threshold: int = 65) -> list:
        ...
        with self.driver.session() as session:
            if category:
                ...
                result = session.run("""
                    ...
                    LIMIT $limit
                """, category=matched_category, limit=fetch_limit)
            elif query:
            ...
            if query and patches:
                ...
                patches = [p for p in patches if p.get('_fuzzy_score', 0) >= fuzzy_threshold][:limit]
    ...
    def rate_patch(self, bank: int, prog: int, rating: int) -> bool:
        ...
        with self.driver.session() as session:
            ...
            """, bank=bank, prog=prog, rating=rating)
        return True
    ...
    return [
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "category": {"type": "string", "description": "Category filter: Bass, Lead, Pad, Drums, etc."},
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "prog": {"type": "integer", "description": "Program number (0-127)"}
                },
                "required": ["bank", "prog"]
            }
        ),
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "bank": {"type": "integer", "description": "Bank number (0-3)"},
                    "prog": {"type": "integer", "description": "Program number (0-127)"},
                    "rating": {"type": "integer", "description": "Rating 1-5 stars", "minimum": 1, "maximum": 5}
                },
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "category": {"type": "string", "description": "Category: Bass, Lead, Pad, Drums, DnB"}
                },
                "required": ["category"]
            }
        ),
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "bpm": {"type": "integer", "description": "Tempo (default 174)", "default": 174},
                    "bass_patch": {"type": "string", "description": "Bass patch name or 'random'", "default": "random"}
                }
            }
        ),
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "bars": {"type": "integer", "description": "Number of bars (1-8)", "default": 4},
                    "pattern": {"type": "string", "enum": ["twostep", "amen", "roller", "jungle"]},
                    "bass_patch": {"type": "string", "enum": ["sub", "reese", "growl"]},
                    "midi_style": {"type": "string", "description": "MIDI style: nosia, pendulum, hospital, etc."}
                }
            }
        ),
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "sc_depth": {"type": "number", "description": "Sidechain depth 0-1"},
                    "sc_release": {"type": "number", "description": "Sidechain release 0.05-0.2s"},
                    "pattern": {"type": "string", "enum": ["twostep", "amen", "roller", "jungle"]},
                    "bass_patch": {"type": "string", "enum": ["sub", "reese", "growl"]},
                    "kick_style": {"type": "string", "enum": ["punchy", "deep", "tight"]}
                }
            }
        ),
        Tool(
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    if name == "virus_search":
        ...
        if not patches:
            return [TextContent(type="text", text=f"No patches found for '{query or category}'")]
        
        # Format results
        lines = [f"🎹 Found {len(patches)} patches:\n"]
        for p in patches:
            rating = "⭐" * p.get("rating", 0) if p.get("rating") else ""
            lines.append(f"  B{p['bank']}P{p['prog']:03d}: {p['name']:12s} [{p.get('category1Name', '?')}] {rating}")
        ...
        if result["success"]:
            p = result["patch"]
            return [TextContent(type="text", text=f"✅ Loaded: {p['name']} (B{bank}P{prog}) - {p.get('category1Name', 'Unknown')}")]
        else:
            return [TextContent(type="text", text=f"❌ Failed to load B{bank}P{prog}: {result.get('error', 'Unknown error')}")]
        ...
        return [TextContent(type="text", text=f"⭐ Rated {patch['name']} as {'⭐' * rating}")]
        ...
        if result["success"]:
            return [TextContent(type="text", text=f"🎲 Random {category}: {patch['name']} (B{patch['bank']}P{patch['prog']})")]
        else:
        ...
        try:
            ...
            return [TextContent(type="text", text=f"🎵 Started DnB jam at {bpm} BPM - Press Ctrl+C in terminal to stop")]
        except Exception as e:
    ...
    # =========================================================================
        ...
        return [TextContent(type="text", text=f"🥁 Played {bars} bars: {PARAMS.pattern} pattern, {PARAMS.bass_patch} bass, sidechain={PARAMS.sc_depth}")]
        ...
        return [TextContent(type="text", text=f"⚡ Updated atomic params: {', '.join(changed)}")]
  ...
  Sidechain:
    ...
    Release: {PARAMS.sc_release:.3f}s"""
        ...
        return [TextContent(type="text", text=f"🎵 Available MIDI styles ({len(styles)}):\n  " + "\n  ".join(styles))]
