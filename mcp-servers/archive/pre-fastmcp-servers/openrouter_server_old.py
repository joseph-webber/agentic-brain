...
MODELS = {
    ...
    "groq-llama-70b": {
        ...
        "key_env": "GROQ_API_KEY", "rate_limit": "30/min"
    },
    "groq-llama-8b": {
...
TASK_ROUTING = {
    ...
    "coding": ["groq-llama-70b", "claude-sonnet", "copilot", "together-llama-70b", "llama3.1:8b"],
    "complex": ["groq-llama-70b", "claude-opus", "together-llama-70b", "claude-sonnet"],
    "general": ["groq-llama-70b", "claude-sonnet", "together-llama-8b", "llama3.1:8b", "gpt-4o"],
    "jira": ["claude-emulator", "claude-sonnet"],
    "accessibility": ["claude-emulator"],
    "private": ["claude-emulator", "llama3.1:8b", "llama3.2:3b"],
    "free": ["groq-llama-70b", "groq-llama-8b", "together-llama-70b", "openrouter-free", "cloudflare-llama"],
            ...
            inputSchema={
                ...
                "properties": {
                    "task": {
                        ...
                        "enum": ["coding", "complex", "quick", "recovery", "private", "general", "jira", "accessibility", "offline", "free", "free-fast"]
                    },
                    "prefer_offline": {
                        "type": "boolean",
...
Or CLI: openrouter use {model.split(':')[0]}"""
        ...
        if model in ["claude", "claude-desktop"]:
            ...
            return [TextContent(type="text", text="🚀 Opening Claude Desktop...\n\nClaude Desktop is starting. You can now use the full Claude experience.")]
        
        elif model == "copilot":
            return [TextContent(type="text", text="🚀 To use Copilot CLI:\n\n```bash\ncopilot\n```\n\nRun this in your terminal.")]
        
        elif model in MODELS or any(model in m for m in MODELS):
            return [TextContent(type="text", text=f"""🚀 To use {model}:
...
• Recovery → claude-emulator"""
        ...
        if info.get("provider") != "ollama":
            return [TextContent(type="text", text=f"⚠️ {model} is a cloud model. Use Claude Desktop or API directly.\n\nFor local chat, try: openrouter_chat(prompt='...', model='claude-emulator')")]
