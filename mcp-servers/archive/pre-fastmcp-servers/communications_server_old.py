...
# Alert keywords
URGENT_KEYWORDS = ['urgent', 'asap', 'immediately', 'today', 'blocker', 'critical', 'emergency']
IMPORTANT_PEOPLE = ['User Two', 'Team Member A', 'Team Member B', 'Team Member C', 'Team Member D']

# Neo4j connection
def get_neo4j():
    return GraphDatabase.driver(
        "bolt://localhost:7687",
        auth=("neo4j", "brain2026")
    )
...
def get_communications_dashboard() -> Dict[str, Any]:
    ...
    with driver.session() as session:
        ...
        dashboard["jira_tickets_active"] = [
            {"ticket": r["ticket"], "mentions": r["mentions"]}
            for r in result
        ]
        
        # Knowledge available
        result = session.run("MATCH (k:CITBKnowledge) RETURN k.topic as topic, k.id as id")
        dashboard["knowledge_available"] = [
            {"topic": r["topic"], "id": r["id"]}
            for r in result
        ]
...
def search_communications(query: str, limit: int = 20, fuzzy_threshold: int = 70) -> Dict[str, Any]:
    ...
    with driver.session() as session:
        ...
        raw_teams = [
            {"sender": r["sender"], "text": r["text"][:300], "time": r["time"], "date": str(r["date"]) if r["date"] else None}
            for r in result
        ]
        # Fuzzy rank results
        for msg in raw_teams:
            score = max(fuzzy_ratio(query, msg["text"]), fuzzy_ratio(query, msg["sender"]))
            msg["_fuzzy_score"] = round(score)
        raw_teams.sort(key=lambda x: x["_fuzzy_score"], reverse=True)
        results["teams_messages"] = [m for m in raw_teams if m["_fuzzy_score"] >= fuzzy_threshold][:limit]
        ...
        """, query=query, limit=fetch_limit)
        raw_emails = [
            {"sender": r["sender"], "subject": r["subject"], "preview": r["preview"][:200] if r["preview"] else None}
            for r in result
        ]
        for email in raw_emails:
            search_text = f"{email['sender']} {email['subject']} {email.get('preview', '')}"
            email["_fuzzy_score"] = round(fuzzy_ratio(query, search_text))
        raw_emails.sort(key=lambda x: x["_fuzzy_score"], reverse=True)
        results["emails"] = [e for e in raw_emails if e["_fuzzy_score"] >= fuzzy_threshold][:limit]
        ...
        """, query=query)
        raw_knowledge = [
            {"topic": r["topic"], "knowledge": r["knowledge"][:500]}
            for r in result
        ]
        for k in raw_knowledge:
            k["_fuzzy_score"] = round(fuzzy_ratio(query, f"{k['topic']} {k['knowledge']}"))
        raw_knowledge.sort(key=lambda x: x["_fuzzy_score"], reverse=True)
        results["knowledge"] = [k for k in raw_knowledge if k["_fuzzy_score"] >= fuzzy_threshold]
...
def get_person_context(person: str) -> Dict[str, Any]:
    ...
    with driver.session() as session:
        ...
        context["messages_sent"] = [
            {"text": r["text"][:300], "time": r["time"], "date": str(r["date"]) if r["date"] else None}
            for r in result
        ]
...
def get_ticket_context(ticket: str) -> Dict[str, Any]:
    ...
    with driver.session() as session:
        ...
        """, ticket=ticket)
        context["messages"] = [
            {"sender": r["sender"], "text": r["text"][:400], "time": r["time"], "date": str(r["date"]) if r["date"] else None}
            for r in result
        ]
        ...
        """, ticket=ticket)
        context["knowledge"] = [
            {"topic": r["topic"], "knowledge": r["knowledge"]}
            for r in result
        ]
...
def get_knowledge(topic: str = None) -> Dict[str, Any]:
    ...
    with driver.session() as session:
        ...
        knowledge["items"] = [
            {"id": r["id"], "topic": r["topic"], "knowledge": r["knowledge"], "source": r["source"]}
            for r in result
        ]
...
def sync_all_channels() -> Dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            [str(BRAIN_ROOT / "venv/bin/python3"), str(BRAIN_ROOT / "tools/teams-safari-sync.py")],
            capture_output=True, text=True, timeout=60, cwd=str(BRAIN_ROOT)
        )
        results["teams"] = "success" if result.returncode == 0 else f"error: {result.stderr[:200]}"
    ...
    try:
        result = subprocess.run(
            [str(BRAIN_ROOT / "venv/bin/python3"), str(BRAIN_ROOT / "tools/outlook-safari-sync.py")],
            capture_output=True, text=True, timeout=60, cwd=str(BRAIN_ROOT)
        )
        results["outlook"] = "success" if result.returncode == 0 else f"error: {result.stderr[:200]}"
    except Exception as e:
    ...
    try:
        result = subprocess.run(
            [str(BRAIN_ROOT / "venv/bin/python3"), "-c", 
             "from core_data.citb_knowledge_extractor import run_extraction; run_extraction()"],
            capture_output=True, text=True, timeout=60, cwd=str(BRAIN_ROOT)
        )
        results["knowledge_extraction"] = "success" if result.returncode == 0 else f"error: {result.stderr[:200]}"
    except Exception as e:
...
def manage_alerts(action: str, keyword: str = None, person: str = None) -> Dict[str, Any]:
    ...
    return {"error": "Invalid action. Use: list, add_keyword, add_person, remove_keyword, remove_person"}
...
def check_for_alerts() -> Dict[str, Any]:
    ...
    with driver.session() as session:
        ...
        for keyword in all_keywords:
            ...
            if count > 0:
                triggered["keyword_matches"].append({"keyword": keyword, "count": count})
        ...
        for person in all_people:
            ...
            if messages:
                triggered["person_matches"].append({"person": person, "recent_messages": messages})
...
def save_comms_state_for_continuity() -> Dict[str, Any]:
    ...
    with driver.session() as session:
        ...
        state["unanswered_questions"] = [
            {"sender": r["sender"], "question": r["text"][:200]}
            for r in result
        ]
    ...
    return [
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "person": {"type": "string", "description": "Person name (e.g., Steve Taylor, Sioban)"}
                },
                "required": ["person"]
            }
        ),
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "topic": {"type": "string", "description": "Optional topic filter (e.g., release, playwright, steve)"}
                },
                "required": []
            }
        ),
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "urgency": {"type": "string", "enum": ["low", "normal", "high"], "description": "Urgency level", "default": "normal"}
                },
                "required": ["message"]
            }
        ),
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "action": {"type": "string", "enum": ["list", "add_keyword", "add_person", "remove_keyword", "remove_person"]},
                    "keyword": {"type": "string", "description": "Keyword to add/remove"},
