    ...
    return [
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "plugin": {"type": "string", "description": "Plugin name (default: Massive)", "default": "Massive"},
                    "note": {"type": "integer", "description": "MIDI note number (0-127, e.g., 36=C1, 48=C2, 60=C3)"},
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "plugin": {"type": "string", "description": "Plugin name", "default": "Massive"},
                    "notes": {"type": "array", "items": {"type": "integer"}, "description": "List of MIDI note numbers"},
                    "velocity": {"type": "integer", "description": "Velocity", "default": 100},
                    "duration": {"type": "number", "description": "Duration in seconds", "default": 1.5}
                },
                "required": ["notes"]
            }
        ),
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "plugin": {"type": "string", "description": "Plugin name", "default": "Massive"},
                    "style": {"type": "string", "description": "Pattern style: roller, halfstep, neurofunk", "default": "roller"},
                    "bars": {"type": "integer", "description": "Number of bars", "default": 4},
                    "bpm": {"type": "integer", "description": "Tempo in BPM", "default": 174},
                    "root": {"type": "integer", "description": "Root note", "default": 33}
                },
                "required": []
            }
        ),
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "plugin": {"type": "string", "description": "Plugin name", "default": "Massive"},
                    "preset_path": {"type": "string", "description": "Path to preset file"}
                },
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "plugin": {"type": "string", "description": "Plugin name", "default": "Massive"},
                    "action": {"type": "string", "description": "list, get, or set", "default": "list"},
                    "param": {"type": "string", "description": "Parameter name (for get/set)"},
                    "value": {"type": "number", "description": "Value to set (for set action)"}
                },
                "required": []
            }
        ),
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "plugin": {"type": "string", "description": "Plugin name", "default": "Massive"},
                    "note": {"type": "integer", "description": "MIDI note to render", "default": 36},
                    "duration": {"type": "number", "description": "Duration in seconds", "default": 2.0},
                    "output": {"type": "string", "description": "Output path", "default": "/tmp/ni_render.wav"}
                },
                "required": []
            }
        ),
...
async def handle_load(name: str) -> List[TextContent]:
    ...
    if not plugin_info:
        return [TextContent(type="text", text=f"❌ Plugin '{name}' not found. Run ni_scan to see available plugins.")]
    
    # Load it
    loader = NIPluginLoader(plugin_info.path)
    _loaded_plugins[plugin_info.name] = loader
    
    return [TextContent(type="text", text=f"✅ Loaded **{plugin_info.name}** ({len(loader.get_parameters())} parameters)")]
...
async def handle_note(plugin: str, note: int, velocity: int, duration: float) -> List[TextContent]:
    """Play a single note."""
    ...
    return [TextContent(type="text", text=f"🎵 Played {note_name} (MIDI {note}) on {plugin} for {duration}s")]


async def handle_chord(plugin: str, notes: List[int], velocity: int, duration: float) -> List[TextContent]:
    """Play a chord."""
    loader = get_or_load_plugin(plugin)
    engine = get_engine()
...
async def handle_pattern(plugin: str, style: str, bars: int, bpm: int, root: int) -> List[TextContent]:
    """Play a DnB pattern."""
    loader = get_or_load_plugin(plugin)
    engine = get_engine()
    ...
    return [TextContent(type="text", text=f"🔊 Played {bars}-bar {style} pattern at {bpm} BPM on {plugin}")]
...
async def handle_params(plugin: str, action: str, param: Optional[str], value: Optional[float]) -> List[TextContent]:
    ...
    return [TextContent(type="text", text="Invalid params action. Use: list, get <param>, set <param> <value>")]


async def handle_render(plugin: str, note: int, duration: float, output: str) -> List[TextContent]:
    """Render to file."""
    loader = get_or_load_plugin(plugin)
    engine = get_engine()
