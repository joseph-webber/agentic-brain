...
def capture_message(content: str, role: str = "user", source: str = "mcp") -> dict:
    ...
    event = hooks.capture_event(event_type=event_type, source=source, content=content, role=role)
    return {"success": True, "event_id": event.event_id, "topics": event.topics, "importance": event.importance}
...
def get_session_context() -> dict:
    ...
    context = [{"role": e.get('role'), "content": e.get('content', '')[:200]} for e in events[-15:]]
    return {"session_id": hooks.session_id, "turn_count": hooks.turn_count, "context": context}
...
def search_all_sessions(query: str, limit: int = 20) -> dict:
    ...
    try:
        with hooks.neo4j_driver.session() as session:
            ...
            matches = [{'session': r['session'], 'role': r['role'], 
                       'content': r['content'][:200] if r['content'] else '', 'topic': r['topic']} for r in result]
