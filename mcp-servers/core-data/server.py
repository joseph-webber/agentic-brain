        ...
        for mod_name, config in EXPOSED_MODULES.items():
            try:
                ...
                for method_name in dir(cls):
                    ...
                    description = f"{config['description']}: {method_name.replace('_', ' ')}"
...
async def list_tools() -> List[Tool]:
    ...
    tools.append(Tool(
        ...
        inputSchema={
            "type": "object",
            "properties": {
                "event_type": {"type": "string", "description": "Type: tool_result, user_message, file_read, error, learning, checkpoint"},
                "summary": {"type": "string", "description": "Brief summary of the context"},
                "full_content": {"type": "string", "description": "Optional full content (stored separately)"},
                "priority": {"type": "string", "description": "critical, high, normal, low, ephemeral (default: normal)"},
                "ttl_hours": {"type": "number", "description": "Custom TTL in hours (overrides priority default)"}
            },
    ...
    tools.append(Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "last_n": {"type": "integer", "description": "Number of items to recall (default: 10)"}
            }
        }
    ))
    tools.append(Tool(
...
async def main():
    """Run the MCP server"""
    print(f"Core Data MCP Server starting with {len(core_data.tools)} tools...", file=sys.stderr)
