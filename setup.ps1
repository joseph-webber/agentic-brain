...
"@ | Out-File -FilePath ".env" -Encoding UTF8
    ...
    # Create .env.dev from example (for Docker)
    if (-not (Test-Path ".env.dev")) {
        if (Test-Path ".env.dev.example") {
            Write-Step "Creating .env.dev from example (for Docker)..."
            Copy-Item ".env.dev.example" ".env.dev"
            Write-Success ".env.dev file created"
            Write-Info "Required for: docker compose -f docker/docker-compose.dev.yml"
        } else {
            Write-Step "Creating .env.dev with defaults..."
            @"
# Agentic Brain Docker Dev Environment
# Generated by setup.ps1 - REQUIRED for docker-compose.dev.yml
...
"@ | Out-File -FilePath ".env.dev" -Encoding UTF8
            Write-Success ".env.dev file created with defaults"
        }
    } else {
        Write-Info ".env.dev already exists"
    }
    
    # Create .env.docker from example
    if (-not (Test-Path ".env.docker")) {
        if (Test-Path ".env.docker.example") {
            Write-Step "Creating .env.docker from example..."
            Copy-Item ".env.docker.example" ".env.docker"
            Write-Success ".env.docker file created"
        }
    }
    ...
    if (-not (Test-Path ".env.test")) {
        if (Test-Path ".env.test.example") {
            Write-Step "Creating .env.test from example..."
            Copy-Item ".env.test.example" ".env.test"
            Write-Success ".env.test file created"
        }
    }
    
    Write-Host ""
    Write-Info "Configuration files created:"
    Write-Info "  .env        - Main config (LLM providers, API keys)"
    Write-Info "  .env.dev    - Docker development (Neo4j, Redis)"
    Write-Info "  .env.docker - Docker production"
    Write-Info "  .env.test   - Test environment"
}
