...
class BaseChunker(ABC):
    ...
    def _add_metadata(self, chunks: List[Chunk], metadata: Optional[Dict[str, Any]]) -> None:
...
class SemanticChunker(BaseChunker):
    ...
    def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        if not sentences:
            return [Chunk(content=text.strip(), start_char=0, end_char=len(text), chunk_index=0)]
...
class RecursiveChunker(BaseChunker):
    ...
    def _recursive_split(
        ...
        good_splits = [s for s in splits if (len(s) > self.min_chunk_size if depth > 0 else True)]
