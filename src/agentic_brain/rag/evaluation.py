...
class RAGEvaluator:
    ...
    def evaluate(
        ...
        for eval_query in dataset.queries:
            ...
            metrics = self._compute_metrics(
                eval_query.query, eval_query.relevant_docs, results, k_values
            )
    ...
    def _compute_metrics(
        ...
        for k in k_values:
            ...
            precision_scores[k] = relevant_retrieved / k if k > 0 else 0.0
    ...
    def _aggregate_metrics(
        self, query_metrics: list[EvalMetrics], k_values: list[int]
    ) -> EvalResults:
        ...
        avg_mrr = (
            statistics.mean([m.mrr for m in query_metrics]) if query_metrics else 0.0
        )
        avg_ndcg = (
            statistics.mean([m.ndcg for m in query_metrics]) if query_metrics else 0.0
        )
        avg_map = (
            statistics.mean([m.map_score for m in query_metrics])
            if query_metrics
            else 0.0
        )
    ...
    def ab_test(
        ...
        comparison = {
            ...
            "improvements": {
                ...
                "precision_at_5": results_b.precision_at_k(5)
                - results_a.precision_at_k(5),
