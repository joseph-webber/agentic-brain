...
class MMRReranker(BaseReranker):
    ...
    def rerank(self, query: str, chunks: list[RetrievedChunk]) -> list[RetrievedChunk]:
        ...
        query_embedding = self.embeddings.embed(query)
        chunk_embeddings = self.embeddings.embed_batch(
                    ...
                    diversity_score = min(
                        [
                            self._cosine_similarity(
                                chunk_embeddings[idx], chunk_embeddings[s]
                            )
...
class CombinedReranker(BaseReranker):
    ...
    def rerank(self, query: str, chunks: list[RetrievedChunk]) -> list[RetrievedChunk]:
        ...
        combined_scores = dict.fromkeys(range(len(chunks)), 0.0)
        ...
        for reranker, weight in self.rerankers:
            ...
            for pos, chunk in enumerate(reranked):
                ...
                for orig_idx, orig_chunk in enumerate(chunks):
                    if (
                        orig_chunk.content == chunk.content
                        and orig_chunk.source == chunk.source
                    ):
        ...
        scored_chunks = [(chunks[i], combined_scores[i]) for i in range(len(chunks))]
