...
Kafka client stack: ``kafka-python`` (lazy import)
--------------------------------------------------
``kafka-python`` is chosen here because RAG loaders are called synchronously
from the pipeline ingestion path — there is no asyncio event loop at that
point, so a sync client is the natural fit. The import is guarded so that the
module is importable even when ``kafka-python`` is not installed.

Other modules use different stacks for specific reasons:
- ``infra/event_bridge.py`` uses ``confluent_kafka`` for low-level bridge control
- ``durability/`` uses ``aiokafka`` for async task/event stores
...
class KafkaLoader(BaseLoader):
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to Kafka at {','.join(self.bootstrap_servers)}")
            return True
...
class RedpandaLoader(BaseLoader):
    ...
    def __init__(
        ...
        self.bootstrap_servers = (
            bootstrap_servers
            or os.environ.get("REDPANDA_BOOTSTRAP_SERVERS", "localhost:9092")
        ).split(",")
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to Redpanda at {','.join(self.bootstrap_servers)}")
            return True
        except Exception as e:
