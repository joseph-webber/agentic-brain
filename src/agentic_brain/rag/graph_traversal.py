...
class GraphTraversalRetriever:
    ...
    def _text_search(
        ...
        # Build text search query with parameterized keywords (SECURITY: prevent injection)
        label_filter = " OR ".join(f"n:{label}" for label in labels)
        # Use parameterized queries instead of string interpolation
        text_conditions = " OR ".join(
            f"toLower(n.content) CONTAINS toLower($kw{i})"
            for i, _ in enumerate(keywords[:5])
        )

        # Build parameters dict for parameterized query
        params: dict[str, Any] = {f"kw{i}": kw for i, kw in enumerate(keywords[:5])}
        params["limit"] = limit
