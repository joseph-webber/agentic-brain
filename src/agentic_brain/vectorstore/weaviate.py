...
try:  # pragma: no cover - optional dependency
    from agentic_brain.vectordb.weaviate_adapter import HAS_WEAVIATE
    from agentic_brain.vectordb.weaviate_adapter import (
        WeaviateAdapter as LegacyWeaviateAdapter,
    )
except Exception:  # pragma: no cover - optional dependency
    HAS_WEAVIATE = False
    LegacyWeaviateAdapter = None  # type: ignore[assignment]


class WeaviateVectorStore(VectorStore):
    """Unified Weaviate-backed vector store."""
    ...
    def connect(self) -> bool:
        if HAS_WEAVIATE and LegacyWeaviateAdapter is not None:
            try:
                self._legacy = LegacyWeaviateAdapter(
                    url=self.url,
                ...
                logger.warning("Weaviate connection failed, using memory fallback: %s", exc)
                self._legacy = None
                self._mode = "memory"
