...
class MaintenanceReport:
    ...
    def is_healthy(self) -> bool:
        ...
        if self.license_report and not self.license_report.is_compliant:
            return False
        return not (
            self.release_report and self.release_report.status == ReleaseStatus.BLOCKED
        )
    ...
    def score(self) -> int:
        ...
        if self.license_report:
            score -= int((100 - self.license_report.compliance_rate) * 0.3)

        if self.doc_report:
            score -= int((100 - self.doc_report.coverage) * 0.2)
    ...
    def __str__(self) -> str:
        ...
        if self.license_report:
            lines.append(
                f"📜 License: {self.license_report.compliance_rate:.0f}% compliant"
            )
    ...
    def full_audit(self) -> MaintenanceReport:
        ...
        if not report.license_report.is_compliant:
            report.issues.append(
                f"{len(report.license_report.missing_header)} files missing Apache-2.0 headers"
            )
        ...
        if report.doc_report.undocumented:
            report.issues.append(
                f"{len(report.doc_report.undocumented)} modules missing docstrings"
            )
    ...
    def fix_all(self, generate_docs: bool = True) -> MaintenanceReport:
        ...
        if report.license_report and not report.license_report.is_compliant:
            ...
            if fixed.fixed_files:
                report.actions_taken.append(
                    f"Added Apache-2.0 headers to {len(fixed.fixed_files)} files"
                )
        ...
        if generate_docs:
            ...
            if doc_report.generated_files:
                report.actions_taken.append(
                    f"Generated {len(doc_report.generated_files)} documentation files"
                )
    ...
    def prepare_release(self, version: str, dry_run: bool = True) -> MaintenanceReport:
        ...
        if release_report.status == ReleaseStatus.READY:
            if not dry_run:
                ...
                report.actions_taken.append(
                    f"[DRY RUN] Would create release v{version}"
                )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command", choices=["audit", "fix", "release", "check"], help="Command to run"
    )
