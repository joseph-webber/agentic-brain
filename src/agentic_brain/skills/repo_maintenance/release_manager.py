...
class ReleaseManager:
    ...
    def check_coverage(self, threshold: int = 80) -> ReleaseCheck:
        ...
        if match:
            ...
            if coverage >= threshold:
                return ReleaseCheck(
                    "Coverage", True, f"{coverage}% coverage (>= {threshold}%)"
                )
            else:
                return ReleaseCheck(
                    "Coverage", False, f"{coverage}% coverage (< {threshold}%)"
                )

        return ReleaseCheck("Coverage", False, "Could not determine coverage")

    def check_license(self) -> ReleaseCheck:
        """Check all files have Apache-2.0 headers."""
        ...
        if report.is_compliant:
            return ReleaseCheck(
                "License",
                True,
                f"All {report.total_files} files have Apache-2.0 headers",
            )
        else:
            return ReleaseCheck(
                "License", False, f"{len(report.missing_header)} files missing headers"
            )
    ...
    def check_docs(self) -> ReleaseCheck:
        ...
        if issues:
            return ReleaseCheck(
                "Documentation", False, ", ".join(issues), blocking=False
            )
    ...
    def check_changelog(self, version: str) -> ReleaseCheck:
        ...
        if version in content:
            return ReleaseCheck(
                "Changelog", True, f"Changelog has entry for v{version}"
            )

        return ReleaseCheck(
            "Changelog", False, f"No changelog entry for v{version}", blocking=False
        )
