...
def get_api_key(config: RouterConfig) -> str:
    ...
    """
    api_key = config.groq_key or os.environ.get("GROQ_API_KEY")
    if not api_key:
...
async def chat_groq(
    ...
    logger.debug(f"Groq chat: model={model}, temperature={temperature}")
    ...
    try:
        status, result = await post_fn(
            url, payload, headers, timeout=config.groq_timeout
        )

        if status == 429:
            raise RateLimitError(limit=30, window="minute", retry_after=60)
        ...
        logger.info(
            f"LLM response received: provider={Provider.GROQ.value}, tokens={tokens_used}"
        )

        # Track token usage
        track_tokens_fn(tokens_used, Provider.GROQ.value)
        ...
        raise LLMProviderError(
            "groq", model, Exception(f"Invalid response from Groq: {e}")
        )
...
async def stream_groq(
    ...
    async with (
        ...
        session.post(
            ...
            timeout=aiohttp.ClientTimeout(total=config.groq_timeout),
        ) as response,
    ):
