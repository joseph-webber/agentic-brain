...
class AgentTask:
    ...
    depends_on: list[str] = field(default_factory=list)  # Agent names that must complete first
...
class CrewConfig:
    ...
    timeout: Optional[float] = None  # Total crew timeout
    verbose: bool = False
    on_task_complete: Optional[Callable[[AgentResult], None]] = None
...
class Crew:
    ...
    def run(
        ...
        if self.config.verbose:
            logger.info(f"Crew starting with {len(self.agents)} agents: {self.strategy.value}")
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.error(f"Error in parallel execution for {task.agent_name}: {e}")
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    # Store failed agent result instead of silently logging
                    logger.error(f"Error in hierarchical execution for {task.agent_name}: {e}")
                    self._results[task.agent_name] = AgentResult(
                        agent_name=task.agent_name,
                        task=task.task,
                        result=None,
    ...
    def get_results(self, agent_name: Optional[str] = None) -> AgentResult | list[AgentResult]:
        """
...
class MockAgent:
    ...
    name: str
    role: str = "worker"
    process_fn: Optional[Callable[[str, dict], str]] = None
