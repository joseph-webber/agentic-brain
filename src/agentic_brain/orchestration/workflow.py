                ...
                if timeout:
                    with ThreadPoolExecutor(max_workers=1) as executor:
                        ...
                        try:
                            ...
                            raise TimeoutError(f"Step '{self.name}' exceeded timeout of {timeout}s")
                else:
...
class WorkflowResult:
    ...
    def get_step_error(self, step_name: str) -> Optional[Exception]:
        ...
        try:
            while current_step_name:
                ...
                if current_step_name in visited_steps:
                    result.state = WorkflowState.FAILED
                    result.error = f"Infinite loop detected at step '{current_step_name}'"
                    break
