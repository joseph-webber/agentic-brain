...
API_URL = os.getenv("AGENTIC_BRAIN_API_URL", "http://localhost:8000")
WS_URL = os.getenv("AGENTIC_BRAIN_WS_URL", "ws://localhost:8000")
...
def example_chat_auto_session() -> Optional[str]:
    ...
    try:
        response = httpx.post(
            f"{API_URL}/chat", json={"message": "What's the weather today?"}
        )
        response.raise_for_status()
...
def example_chat_persistent_session(session_id: str) -> None:
    ...
    messages = ["How are you?", "What time is it?", "Tell me a joke!"]
...
def example_get_session_messages(session_id: str) -> None:
    ...
    try:
        response = httpx.get(
            f"{API_URL}/session/{session_id}/messages", params={"limit": 10}
        )
        response.raise_for_status()
...
def example_delete_session(session_id: str) -> None:
    ...
    try:
        ...
        if e.response.status_code == 404:
            print("✓ Session already deleted or doesn't exist")
        else:
            print(f"✗ Error: {e}")
...
def example_error_handling() -> None:
    ...
    try:
        response = httpx.post(f"{API_URL}/chat", json={"message": ""})
        response.raise_for_status()
    except httpx.HTTPStatusError as e:
    ...
    try:
        response = httpx.post(
            f"{API_URL}/chat", json={"user_id": "test"}  # Missing 'message'
        )
        response.raise_for_status()
    except httpx.HTTPStatusError as e:
...
async def example_websocket_chat() -> None:
    ...
    try:
        async with websockets.connect(f"{WS_URL}/ws/chat") as websocket:
            ...
            # Send messages
            test_messages = ["Hello bot!", "How are you?", "What time is it?"]
...
def example_multi_user_sessions() -> None:
    ...
    try:
        ...
        for user in users:
            response = httpx.post(
                f"{API_URL}/chat",
                json={"message": f"Hello, I am {user['name']}", "user_id": user["id"]},
            )
            response.raise_for_status()
...
def example_continuous_conversation() -> None:
    ...
    try:
        ...
        for i, message in enumerate(conversation, 1):
            response = httpx.post(
                f"{API_URL}/chat", json={"message": message, "session_id": session_id}
            )
            response.raise_for_status()
        ...
        if session_id:
            ...
            print(
                f"  Session duration: {data['created_at']} to {data['last_accessed']}"
            )
