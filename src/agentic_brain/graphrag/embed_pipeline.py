...
def _get_openai_key() -> Optional[str]:
    ...
    try:
        result = subprocess.run(
            ["security", "find-generic-password", "-s", "agentic-brain", "-a", "openai-api-key", "-w"],
...
def embed_sessions(dry_run: bool = False, force: bool = False, session_id: Optional[str] = None):
    """
    ...
    try:
        ...
        print(f"  Ollama {OLLAMA_MODEL}: {'available' if has_ollama else 'NOT available — will fall back to OpenAI'}")
        
        if not has_ollama:
            print(f"  TIP: Run `ollama pull {OLLAMA_MODEL}` to enable local embeddings.")
...
def main():
    ...
    parser.add_argument("--force", action="store_true", help="Re-embed all sessions, even those already embedded.")
    parser.add_argument("--session-id", metavar="ID", help="Embed a single session only.")
