...
class UsageTracker:
    ...
    def get_daily_stats(self, date: str, bot_name: Optional[str] = None) -> DailyUsageStats:
        """
        ...
        if bot_name:
            ...
            """
            params = {"date": date, "bot_name": bot_name}
        else:
            query = """
            WITH date($date) as target_date,
                 duration.inDays(date($date), date($date) + duration({days: 1})).days as day_duration
    ...
    def get_weekly_stats(
        ...
        return {
            ...
            "avg_response_time_ms": sum(s.avg_response_time_ms for s in daily_stats) / len(daily_stats),
            "avg_error_rate_pct": sum(s.error_rate_pct for s in daily_stats) / len(daily_stats),
    ...
    def get_monthly_stats(
        ...
        return {
            ...
            "avg_response_time_ms": sum(w["avg_response_time_ms"] for w in weekly_data) / len(weekly_data),
            "weeks": weekly_data,
        }
