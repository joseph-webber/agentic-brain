...
class APIEndpoint:
    ...
    def __post_init__(self):
        ...
        if self.requires_ssl and not self.base_url.startswith("https://"):
            raise ValueError(
                f"API endpoint {self.name} requires SSL but URL is not HTTPS"
            )
...
class APIAccessController:
    ...
    def register_api(self, endpoint: APIEndpoint, credentials: Dict[str, str]) -> None:
        """
        ...
        self.rate_limiters[endpoint.name] = RateLimiter(
            max_requests=endpoint.rate_limit, window_seconds=60
        )
    ...
    def _build_auth_headers(
        self, endpoint: APIEndpoint, credentials: Dict[str, str]
    ) -> Dict[str, str]:
    ...
    async def call_api(
        ...
        auth_headers = self._build_auth_headers(endpoint, self.api_keys[api_name])
    ...
    def get_access_log(
        self, api_name: Optional[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
