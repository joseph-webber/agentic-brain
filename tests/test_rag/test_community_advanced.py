...
class FakeResult:
    def __init__(self, records: list[dict] | None = None, single_record: dict | None = None):
        self.records = records or []
        self.single_record = single_record if single_record is not None else (self.records[0] if self.records else None)
...
def make_sample_hierarchy(modules):
    hierarchy = modules.detection.CommunityHierarchy(levels=3, detection_method="leiden_hierarchical")
    hierarchy.communities[1] = modules.detection.Community(id=1, level=0, entities=["alice", "bob"], summary="People cluster")
    hierarchy.communities[2] = modules.detection.Community(id=2, level=0, entities=["neo4j", "cypher"], summary="Graph cluster")
    hierarchy.communities[3] = modules.detection.Community(id=3, level=1, entities=["alice", "bob", "neo4j", "cypher"], children_ids=[1, 2])
    hierarchy.communities[4] = modules.detection.Community(id=4, level=2, entities=["alice", "bob", "neo4j", "cypher"], children_ids=[3])
...
class TestCommunityDetectionAdvanced:
    ...
    def test_leiden_uses_resolution_iterations_and_randomness(self, modules):
        ...
        leiden_calls = [call for call in session.calls if "CALL gds.leiden.stream" in call[0]]
    ...
    def test_leiden_supports_four_plus_levels(self, modules):
        session = SyncSession(
            ...
            edge_records=[{"source": "a", "target": "b"}, {"source": "c", "target": "d"}],
        )
        hierarchy = modules.detection._detect_leiden_hierarchical(session, resolution=1.0, max_levels=4)
    ...
    def test_leiden_skips_duplicate_partitions(self, modules):
        ...
        session = SyncSession(
            leiden_by_gamma={8.0: shared_partition, 4.0: shared_partition, 2.0: shared_partition},
            edge_records=[],
        )
        hierarchy = modules.detection._detect_leiden_hierarchical(session, resolution=2.0, max_levels=3)
        assert hierarchy.levels == 1

    def test_leiden_wires_parent_child_relationships(self, modules):
        session = SyncSession(
            leiden_by_gamma={
                2.0: [{"entity": "a", "communityId": 1}, {"entity": "b", "communityId": 2}],
                1.0: [{"entity": "a", "communityId": 1}, {"entity": "b", "communityId": 1}],
            },
            edge_records=[],
        )
        hierarchy = modules.detection._detect_leiden_hierarchical(session, resolution=1.0, max_levels=2)
        ...
        assert all(community.parent_id == parent_nodes[0].id for community in leaf_nodes)

    def test_leiden_entity_mapping_uses_leaf_level(self, modules):
        session = SyncSession(
            leiden_by_gamma={1.0: [{"entity": "a", "communityId": 7}, {"entity": "b", "communityId": 7}]},
            edge_records=[],
        )
        hierarchy = modules.detection._detect_leiden_hierarchical(session, resolution=1.0, max_levels=1)
    ...
    def test_leiden_level_metrics_include_modularity_coverage_cohesion(self, modules):
        session = SyncSession(
            leiden_by_gamma={1.0: [{"entity": "a", "communityId": 1}, {"entity": "b", "communityId": 1}]},
    ...
    def test_resolution_override_takes_precedence(self, modules):
        session = SyncSession(leiden_by_gamma={3.0: [{"entity": "a", "communityId": 1}]}, edge_records=[])
        modules.detection._detect_leiden_hierarchical(session, gamma=1.0, resolution=3.0, max_levels=1)
        leiden_call = [call for call in session.calls if "CALL gds.leiden.stream" in call[0]][0]
        assert leiden_call[1]["gamma"] == 3.0

    def test_detect_hierarchical_can_be_disabled(self, modules):
        hierarchy = modules.detection.detect_communities_hierarchical(object(), enabled=False)
    ...
    def test_detect_hierarchical_falls_back_to_louvain(self, modules):
        session = SyncSession(
            ...
            louvain_records=[{"entity": "a", "communityId": 1}, {"entity": "b", "communityId": 1}],
            edge_records=[],
        )
        hierarchy = modules.detection.detect_communities_hierarchical(session, resolution=8.0, max_levels=1)
        assert hierarchy.detection_method == "louvain"

    def test_detect_hierarchical_falls_back_without_gds(self, modules):
        session = SyncSession(
            gds_available=False,
            component_records=[{"entity": "a", "community_key": "group_a"}, {"entity": "b", "community_key": "group_a"}],
            edge_records=[],
        )
    ...
    def test_summarize_community_structured_fallback_includes_metrics(self, modules):
        ...
        session = SyncSession(entity_detail_records=[{"name": "alice", "type": "Person", "mentions": 2, "rels": []}])
    ...
    def test_summarize_parent_community_uses_child_summaries(self, modules):
        community = modules.detection.Community(id=4, level=2, entities=["alice", "neo4j"], children_ids=[1, 2])
    ...
    def test_summarize_community_uses_llm_generate(self, modules):
        ...
        session = SyncSession(entity_detail_records=[{"name": "alice", "type": "Person", "mentions": 1, "rels": []}])
        summary = modules.detection.summarize_community(session, community, llm=StubLLM())
    ...
    def test_detect_communities_returns_flat_map(self, modules):
        ...
        flat = modules.detection.detect_communities(session, resolution=1.0, max_levels=1)
        assert any("a" in members for members in flat.values())

    def test_async_detect_hierarchical_disabled(self, modules):
        hierarchy = asyncio.run(modules.detection.detect_communities_hierarchical_async(object(), enabled=False))
        assert hierarchy.detection_method == "disabled"
...
class TestCommunityGraphRAGAdvanced:
    def test_route_query_global_prefers_community(self, modules):
        ...
        assert asyncio.run(rag.route_query("Summarize the overall themes across all communities")) == "community"
    ...
    def test_route_query_mixed_prefers_hybrid(self, modules):
        rag = modules.community.CommunityGraphRAG(AsyncDriver())
        assert asyncio.run(rag.route_query("How does Alice connect to Neo4j architecture")) == "hybrid"

    def test_route_query_with_communities_disabled_never_returns_community(self, modules):
        rag = modules.community.CommunityGraphRAG(AsyncDriver(), enable_communities=False)
    ...
    def test_select_optimal_level_global_uses_highest_level(self, modules):
        ...
        rag._compose_hierarchy_levels = lambda: asyncio.sleep(0, result={0: [], 1: [1], 2: [1], 3: [1], 4: [1]})
        assert asyncio.run(rag.select_optimal_level("overall themes across everything")) == 4

    def test_select_optimal_level_local_uses_lowest_level(self, modules):
        rag = modules.community.CommunityGraphRAG(AsyncDriver(), max_hierarchy_levels=4)
        rag._compose_hierarchy_levels = lambda: asyncio.sleep(0, result={0: [], 1: [1], 2: [1], 3: [1]})
    ...
    def test_select_optimal_level_scales_with_complexity(self, modules):
        ...
        complex_level = asyncio.run(rag.select_optimal_level("compare relationships and patterns across teams and platforms"))
    ...
    def test_compose_hierarchy_builds_four_plus_levels(self, modules):
        rag = modules.community.CommunityGraphRAG(AsyncDriver(), max_hierarchy_levels=4)
        rag._get_leaf_communities = lambda: asyncio.sleep(0, result=make_leaf_levels(modules, 16))
    ...
    def test_build_hierarchy_persists_nodes(self, modules):
        ...
        rag._get_leaf_communities = lambda: asyncio.sleep(0, result=make_leaf_levels(modules, 8))
    ...
    def test_query_community_returns_selected_level(self, modules):
        ...
        rag._search_community_summaries = lambda query, top_k, hierarchy_level=None: asyncio.sleep(
            0,
    ...
    def test_query_entity_returns_hierarchy_level_zero(self, modules):
        ...
        rag._search_entities = lambda query, top_k: asyncio.sleep(0, result=[{"entity_name": "alice"}])
    ...
    def test_query_level_delegates_to_summary_search(self, modules):
        rag = modules.community.CommunityGraphRAG(AsyncDriver())
        rag._search_community_summaries = lambda query, top_k, hierarchy_level=None: asyncio.sleep(0, result=[{"level": hierarchy_level}])
    ...
    def test_estimate_query_complexity_higher_for_complex_query(self, modules):
        ...
        complex_query = rag._estimate_query_complexity("compare relationships and patterns across all communities")
    ...
    def test_search_community_summaries_falls_back_to_in_memory_levels(self, modules):
        ...
        rag._compose_hierarchy_levels = lambda: asyncio.sleep(
            ...
            result={
                0: [],
                1: [modules.community.CommunityLevel("leaf", 1, ["alice"], "Alice summary")],
                2: [modules.community.CommunityLevel("global", 2, ["alice", "neo4j"], "Global summary")],
            },
        )
        results = asyncio.run(rag._search_community_summaries("alice", 5, hierarchy_level=1))
    ...
    def test_rank_community_records_orders_by_score_and_size(self, modules):
        ...
        ranked = rag._rank_community_records(
            [
                {"community_id": "small", "level": 1, "summary": "neo4j", "members": ["neo4j"], "member_count": 1},
                {"community_id": "large", "level": 1, "summary": "neo4j graph", "members": ["neo4j", "graph"], "member_count": 2},
            ],
            ["neo4j", "graph"],
        )
    ...
    def test_query_result_includes_query_complexity(self, modules):
        result = modules.community.CommunityQueryResult(strategy="hybrid", results=[], hierarchy_level=1, query_complexity=0.75)
