...
class TestTransportMessage:
    ...
    def test_message_custom_timestamp(self):
        ...
        custom_ts = datetime(2024, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        msg = TransportMessage(
            content="Test",
            session_id="s1",
            message_id="m1",
...
class TestTransportType:
    ...
    def test_transport_type_enum_members(self):
        """Test that both expected transport types exist."""
...
class TestWebSocketTransport:
    ...
    @pytest.fixture
    def transport(self, config, mock_websocket):
        """Create a WebSocket transport."""
        return WebSocketTransport(config, mock_websocket)
    ...
    @pytest.mark.asyncio
    async def test_connect_no_websocket(self, config):
        """Test connection without WebSocket instance."""
        transport = WebSocketTransport(config, None)
        result = await transport.connect()
        assert result is False
        assert transport.connected is False
    
    @pytest.mark.asyncio
    async def test_connect_exception(self, config, mock_websocket):
        """Test connection with exception."""
        mock_websocket.accept.side_effect = Exception("Connection failed")
        transport = WebSocketTransport(config, mock_websocket)
        result = await transport.connect()
        assert result is False
    ...
    @pytest.mark.asyncio
    async def test_send_without_websocket(self, config):
...
class TestWebSocketReceive:
    ...
    async def test_receive_basic_message(self):
        ...
        config = TransportConfig()
        transport = WebSocketTransport(config, mock_ws)
        await transport.connect()
...
class TestTransportMessageTimestamps:
    ...
    def test_message_serialization_timestamp(self):
        ...
        assert reconstructed.tzinfo == timezone.utc
