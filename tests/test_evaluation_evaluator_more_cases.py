...
def test_evaluator_with_no_retrievals_and_no_answers():
    ds = Dataset.from_list([{"id":"1","question":"q","gold_answer":"a","gold_context_ids":["c1"]}])
    ev = RAGEvaluator()
    rep = ev.evaluate(ds)
    s = rep.summary()
...
def test_evaluator_with_partial_scores():
    ...
    ev = RAGEvaluator()
    rep = ev.evaluate(ds, retrievals=[["c1"],["c2"]], retrieved_scores=[[1.0],[0.5]], generated_answers=["a","x"])
    s = rep.summary()
