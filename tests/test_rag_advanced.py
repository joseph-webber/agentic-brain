...
from agentic_brain.rag.hybrid import HybridSearch, BM25Index
from agentic_brain.rag.evaluation import EvalDataset, EvalQuery, RAGEvaluator
...
"""
    ...
    def test_fixed_chunker(self, sample_text):
        ...
        if len(chunks) > 1:
            ...
            next_chunk_start = chunks[1].content[:50]
...
class TestReranking:
    ...
    def test_combined_reranker(self, sample_chunks):
        ...
        combined = CombinedReranker(
            rerankers=[(reranker1, 1.0), (reranker2, 1.0)],
            top_k=3
        )
...
class TestEvaluation:
    ...
    def test_rag_evaluator(self, eval_dataset):
        ...
        def mock_retriever(query: str):
            ...
            return [
                RetrievedChunk(content="ml_guide content", source="ml_guide.txt", score=0.9),
