...
class TestBaseSecureAgent:
    ...
    def test_audit_logging(self):
        ...
        blocked = [
            e for e in audit if e["action"] == "execute_command" and not e["allowed"]
        ]
    ...
    def test_agent_id_in_logs(self):
        ...
        agent = TestSecureAgent(
            security_role=SecurityRole.USER, agent_id="test-agent-123"
        )
...
class TestEventProcessor:
    ...
    def test_processor_handle_event(self):
        ...
        event = {"type": "order.created", "data": {"order_id": 12345}}
...
class TestSecurityIntegration:
    ...
    def test_multi_agent_scenario(self):
        ...
        chatbot = TaskAgent(
            security_role=SecurityRole.USER, agent_id="customer-chatbot"
        )

        # Developer tool (SAFE_ADMIN role)
        dev_tool = ExploreAgent(
            security_role=SecurityRole.SAFE_ADMIN, agent_id="dev-explorer"
        )

        # System monitor (FULL_ADMIN role)
        monitor = BackgroundWorker(
            security_role=SecurityRole.FULL_ADMIN, agent_id="system-monitor"
        )
    ...
    def test_security_context_isolation(self):
        ...
        agent1 = TestSecureAgent(security_role=SecurityRole.USER, agent_id="agent-1")
        agent2 = TestSecureAgent(
            security_role=SecurityRole.SAFE_ADMIN, agent_id="agent-2"
        )
