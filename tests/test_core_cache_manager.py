...
def test_cache_get_cached_returns_none_when_no_record(monkeypatch):
    monkeypatch.setattr(cm, "get_session", lambda database=None: DummyCtx(DummySession(None)))
    c = cm.CacheManager()
...
def test_cache_get_cached_returns_json_when_present(monkeypatch):
    ...
    monkeypatch.setattr(cm, "get_session", lambda database=None: DummyCtx(DummySession(rec)))
    c = cm.CacheManager()
