...
def patch_get_session(monkeypatch, recorder):
    ...
    monkeypatch.setattr(pool, "get_session", lambda database=None: fake_get_session(recorder))
...
def test_store_messages_bulk_runs_unwind(monkeypatch):
    ...
    assert stored == 3
    assert any("UNWIND $rows AS r" in q[0] or "UNWIND $rows AS r" in (q[0] if isinstance(q[0], str) else "") for q in calls)
...
def test_conversation_memory_add_messages_batch_uses_unwind(monkeypatch):
    ...
    monkeypatch.setattr(pool, "get_session", lambda database=None: fake_get_session(recorded))
    ...
    assert res == 2
    assert any("UNWIND $rows AS r" in (q[0] if isinstance(q[0], str) else "") for q in recorded)
...
def test_get_conversation_history_pagination_uses_skip_limit(monkeypatch):
    ...
    def fake_resilient(session, query, params=None):
        ...
        return [
            {"id": "m1", "role": "user", "content": "x", "timestamp": sm.datetime.now(sm.UTC).isoformat(), "session_id": "s1", "metadata": "{}", "entities": []}
        ]
    ...
    msgs = asyncio.get_event_loop().run_until_complete(conv.get_conversation_history(page=1, page_size=5))
...
def test_batch_add_returns_zero_for_empty(monkeypatch):
    recorded = []
    monkeypatch.setattr(nm, "resilient_query_sync", lambda s, q, p=None: recorded.append((q, p)) or [])
    import agentic_brain.core.neo4j_pool as pool
    monkeypatch.setattr(pool, "get_session", lambda database=None: fake_get_session(recorded))
...
def test_get_conversation_history_since_filter(monkeypatch):
    ...
    import asyncio
    asyncio.get_event_loop().run_until_complete(conv.get_conversation_history(since=since))
    q, params = recorded[-1]
