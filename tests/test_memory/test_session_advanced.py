...
async def session_with_messages(session):
    ...
    await session.add_message("assistant", "SD-1350 is currently in review by Steve Taylor.")
...
async def manager_with_sessions(manager):
    ...
    await session1.add_message("assistant", "Here's how to implement binary search in Python...")
...
class TestSessionCompression:
    ...
    async def test_compress_reduces_tokens(self, session):
        ...
        # Add many messages
        for i in range(30):
            await session.add_message("user", f"This is message number {i} with some content.")
            await session.add_message("assistant", f"Response to message {i} with details.")
    ...
    async def test_compress_keeps_important(self, session):
        ...
        await session.add_message("user", "This is important! Remember this critical fact.")
        await session.add_message("assistant", "I'll remember this important information.")
...
class TestSessionBranching:
    ...
    async def test_branch_at_specific_index(self, manager_with_sessions):
        ...
        branched = await manager_with_sessions.branch_session("session-tech", branch_at_index=2)
    ...
    async def test_branch_with_custom_id(self, manager_with_sessions):
        ...
        branched = await manager_with_sessions.branch_session(
            "session-tech",
            new_session_id="my-custom-branch"
        )
...
class TestSessionMerging:
    ...
    async def test_merge_interleave(self, manager_with_sessions):
        ...
        assert merged._session_metadata["merged_from"] == ["session-tech", "session-project"]
...
class TestSessionReplay:
    ...
    async def test_replay_inject_messages(self, manager_with_sessions):
        ...
        config = ReplayConfig(
            inject_messages={
                1: [{"role": "system", "content": "Injected instruction"}]
            }
        )
    ...
    async def test_replay_modify_system_prompt(self, manager):
        ...
        replayed = await manager.replay_session(
            "prompt-test",
            replay_config=config
        )
