...
def test_suite_runs_and_reports_metrics() -> None:
    suite = BenchmarkSuite(rag_iterations=4, embedding_iterations=6, graph_iterations=5, request_count=8, concurrency=4)
    result = suite.run()
...
def test_suite_table_and_markdown_render() -> None:
    suite = BenchmarkSuite(rag_iterations=2, embedding_iterations=2, graph_iterations=2, request_count=2)
    result = suite.run()
