...
class TestWorkflow:
    ...
    def test_workflow_add_steps_fluent(self):
        ...
        result = (
            workflow
            .add_step(WorkflowStep(name="step1", execute=execute_fn))
    ...
    def test_workflow_simple_execution(self):
        ...
        workflow.add_step(
            WorkflowStep(name="step2", execute=step2_fn)
        )
...
class TestIntegration:
    ...
    def test_crew_with_workflow(self):
        ...
        def workflow_fn(ctx):
            crew = Crew(
                [
                    MockAgent(name="a", process_fn=lambda t, c: (agents_ran.append("a"), "a_result")[1]),
                    MockAgent(name="b", process_fn=lambda t, c: (agents_ran.append("b"), "b_result")[1]),
                ]
            )
    ...
    def test_workflow_with_conditional_crew(self):
        ...
        workflow.add_step(
            WorkflowStep(
                ...
                transitions=[Transition(to_step="run_seq"), Transition(to_step="run_par")]
            )
        )
        workflow.add_step(
            WorkflowStep(name="run_seq", execute=run_crew_sequential)
        )
        workflow.add_step(
            WorkflowStep(name="run_par", execute=run_crew_parallel)
        )
