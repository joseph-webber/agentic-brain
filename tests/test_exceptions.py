...
class TestAgenticBrainError:
    ...
    def test_exception_attributes_accessible(self):
        ...
        exc = AgenticBrainError(
            message="Test", cause="Cause", fix="Fix", debug_info={"key": "value"}
        )
...
class TestLLMProviderError:
    ...
    @pytest.mark.parametrize(
        "provider,model,expected_in_msg",
        [
            ("ollama", "llama3.2:3b", ["ollama", "ollama serve", "ollama pull"]),
            ("openai", "gpt-4o", ["openai", "OPENAI_API_KEY", "platform.openai.com"]),
            (
                "anthropic",
                "claude-3-sonnet",
            ...
            ("unknown", "model", ["unknown", "UNKNOWN_API_KEY"]),
        ],
    )
    def test_provider_error_messages(self, provider, model, expected_in_msg):
        """Test error messages contain provider-specific information."""
        exc = LLMProviderError(provider, model)
        msg = str(exc)
...
class TestConfigurationError:
    ...
    def test_configuration_error_with_example(self):
        ...
        exc = ConfigurationError(
            "API_KEY", "32-character hex string", "a1b2c3d4e5f6..."
        )
        msg = str(exc)
...
class TestAPIError:
    ...
    def test_api_error_with_original_error(self):
        ...
        exc = APIError(
            "https://api.example.com/data", 503, "Service Unavailable", original
        )
        msg = str(exc)
...
class TestExceptionMessages:
    ...
    def test_all_exceptions_have_actionable_fixes(self):
        ...
        for exc in exceptions_with_fixes:
            ...
            assert any(
                keyword in exc.fix.lower()
                for keyword in ["check", "verify", "try", "set", "ensure", "✓"]
            )
