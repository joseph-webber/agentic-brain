...
class TestChatConfig:
    ...
    def test_default_config_values(self, attr, expected):
        """Test default configuration values."""
        config = ChatConfig()
        assert getattr(config, attr) == expected

    @pytest.mark.parametrize(
        "preset_method,attr,expected",
        [
            ("minimal", "persist_sessions", False),
            ("minimal", "use_memory", False),
            ("minimal", "max_history", 20),
            ("business", "customer_isolation", True),
            ("business", "persist_sessions", True),
            ("business", "use_memory", True),
            ("business", "session_timeout", 7200),
        ],
    )
    def test_config_presets(self, preset_method, attr, expected):
        """Test ChatConfig preset methods."""
        config = getattr(ChatConfig, preset_method)()
        assert getattr(config, attr) == expected
    ...
    def test_session_dir_created(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ChatConfig(session_dir=session_dir, persist_sessions=True)
...
class TestSession:
    ...
    def test_session_with_user_id(self):
        ...
        session = Session(
            session_id="user_chat", user_id="user_123", bot_name="support"
        )
    ...
    def test_add_message_with_metadata(self):
        ...
        msg = session.add_message("user", "Hello", token_count=5, model="gpt-4")
    ...
    def test_session_updated_at_changes(self):
        ...
        with patch("agentic_brain.chat.session.datetime") as mock_dt:
    ...
    def test_session_to_dict(self):
        ...
        session = Session(session_id="test", user_id="user_1", bot_name="bot")
        session.add_message("user", "Hello")
...
class TestSessionManager:
    ...
    def test_cleanup_expired_sessions(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manager2 = SessionManager(
                Path(tmpdir), timeout_seconds=1, auto_cleanup=True
            )
    ...
    def test_cleanup_preserves_recent_sessions(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            manager = SessionManager(
                Path(tmpdir), timeout_seconds=3600, auto_cleanup=False
            )
            ...
            # Create new manager with cleanup
            manager2 = SessionManager(
                Path(tmpdir), timeout_seconds=3600, auto_cleanup=True
            )
...
class TestChatMessage:
    ...
    def test_chat_message_to_dict(self):
        ...
        msg = ChatMessage(role="user", content="Hello", metadata={"key": "value"})
...
class TestChatbot:
    ...
    def test_chatbot_with_persistence_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            config = ChatConfig(persist_sessions=True, session_dir=Path(tmpdir))
            bot = Chatbot("bot", config=config)

            assert bot.session_manager is not None

    @patch("agentic_brain.chat.chatbot.Chatbot._call_llm_async", new_callable=AsyncMock)
    ...
    def test_chat_with_metadata(self, mock_llm):
        ...
        bot.chat("Hello", metadata={"source": "api", "ip": "192.168.1.1"})
...
class TestChatbotIntegration:
    ...
    def test_full_conversation_flow(self, mock_llm):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Conversation
            bot.chat("Hello, I need help")
            bot.chat("Can you assist me?")
            bot.chat("No, that's all")
