...
from agentic_brain.streaming import StreamingResponse, StreamProvider, StreamToken
...
class TestStreamToken:
    ...
    def test_create_token(self):
        ...
        token = StreamToken(
            token="hello", is_start=True, metadata={"provider": "ollama"}
        )
...
class TestStreamingResponse:
    ...
    def test_init_openai_missing_key(self):
        ...
        with patch.dict("os.environ", {}, clear=True):
            with pytest.raises(ValueError, match="OPENAI_API_KEY required"):
                StreamingResponse(provider="openai", model="gpt-4")
    ...
    def test_init_anthropic_missing_key(self):
        ...
        with patch.dict("os.environ", {}, clear=True):
            with pytest.raises(ValueError, match="ANTHROPIC_API_KEY required"):
                StreamingResponse(provider="anthropic", model="claude-3-sonnet")

    def test_init_with_env_vars(self):
        """Test initialization using environment variables."""
        with patch.dict("os.environ", {"OPENAI_API_KEY": "sk-test-123"}):
            streamer = StreamingResponse(provider="openai", model="gpt-4")

            assert streamer.api_key == "sk-test-123"

    def test_custom_api_base(self):
        """Test setting custom API base."""
        streamer = StreamingResponse(provider="ollama", api_base="http://custom:11434")
    ...
    def test_custom_system_prompt(self):
        ...
        streamer = StreamingResponse(provider="ollama", system_prompt=custom_prompt)
...
class TestErrorHandling:
    ...
    async def test_stream_error_handling(self):
        ...
        with pytest.raises(Exception, match="Connection failed"):
            async for _token in streamer.stream("test"):
                pass
