...
This project enforces code quality and license compliance through automated tools:
...
- All new Python files in `src/` **must** begin with the Apache 2.0 header:
  ...
  ```
- Do **not** introduce other copyleft licenses (GPL, LGPL, AGPL) into this repository without explicit approval.
- The CI `License Check` workflow and `scripts/check_license_headers.py` will fail if headers are missing or incorrect.
...
- **Bug Report**: For reporting bugs — see `.github/ISSUE_TEMPLATE/bug_report.md`
- **Feature Request**: For suggesting improvements — see `.github/ISSUE_TEMPLATE/feature_request.md`

These templates ensure all necessary information is captured upfront.
