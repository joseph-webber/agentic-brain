...
```mermaid
graph LR
    A[Admin Query] --> B(Intent Analysis)
    B --> C{RAG Query}
    C -->|Sales Data| D[Order History DB]
    C -->|Stock Levels| E[Inventory API]
    D & E --> F[LLM Synthesis]
    F --> G[Actionable Insight]
```
...
graph LR
    ...
    C -->|Product Vectors| D[Vector DB]
    C -->|User Context| E[Session/History]
    D & E --> F[Ranking & Filtering]
