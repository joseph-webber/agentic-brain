# 📚 Documentation Index

Complete reference for the Agentic Brain documentation.

## 🚀 Getting Started
- [**Quick Start**](QUICKSTART.md) - Get up and running in 5 minutes
- [**Setup Guide**](SETUP.md) - Detailed setup instructions
- [**Installation Options**](INSTALL_OPTIONS.md) - Available installation paths
- [**Configuration**](configuration.md) - Environment variables and runtime settings
- [**System Requirements**](SYSTEM_REQUIREMENTS.md) - Supported runtimes and prerequisites
- [**Troubleshooting**](TROUBLESHOOTING.md) - Common issues and solutions

## 📖 API & Reference
- [**API Reference**](API_REFERENCE.md) - Core API endpoints and usage
- [**API Overview**](API.md) - General API information
- [**Agent API**](api/agent.md) - Agent API reference
- [**Memory API**](api/memory.md) - Memory API reference
- [**RAG API**](api/rag.md) - RAG API reference

## 🏗️ Architecture & Core Concepts
- [**Architecture Overview**](architecture.md) - High-level system design
- [**Memory System**](MEMORY.md) - Persistent memory architecture
- [**RAG Guide**](RAG_GUIDE.md) - Practical RAG guidance
- [**LLM Guide**](LLM_GUIDE.md) - Working with models and providers
- [**Vector Embeddings**](VECTOR_EMBEDDINGS.md) - Embedding architecture

## 💻 Platform & Environment Guides
- [**macOS Development Guide**](MACOS_DEVELOPMENT.md) - Main guide for macOS development
- [**Windows Installation**](WINDOWS_INSTALL.md) - Native Windows setup
- [**Platform Support**](PLATFORM_SUPPORT.md) - Supported OS, CPU, and hardware combinations

## 🔌 Integrations
- [**Enhanced Integrations**](INTEGRATIONS.md) - Mem0, LiteLLM, Docling, neo4j-graphrag, Chonkie
- [**Voice Integration Guide**](VOICE_INTEGRATION_GUIDE.md) - Adding speech capabilities
- [**Neo4j Integration**](integrations/NEO4J.md) - Neo4j integration details
- [**JHipster Integration**](integrations/JHIPSTER.md) - JHipster integration guidance

## 🛡️ Security & Compliance
- [**Security Policy**](SECURITY.md) - Core security policy
- [**Accessibility**](ACCESSIBILITY.md) - WCAG compliance expectations

## 📈 Roadmaps & Strategy
- [**Roadmap**](ROADMAP.md) - Product roadmap
- [**Growth Strategy**](GROWTH_STRATEGY.md) - Product growth plan

---
Maintained manually to keep README links and the docs tree aligned.
