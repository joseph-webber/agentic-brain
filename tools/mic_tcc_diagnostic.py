...
def check_sounddevice():
    try:
        ...
        if peak < 1e-10:
            print(
                f"  ⚠  Peak amplitude: {peak:.2e} — likely ZERO (permission denied at driver level)"
            )
        else:
...
def main():
    ...
    status_line = run(
        'swift -e "import AVFoundation; print(AVCaptureDevice.authorizationStatus(for:.audio).rawValue)" 2>/dev/null'
    ).strip()
    if status_line == "3":  # authorized
        ...
        print(
            "     The app now requests microphone permission automatically on launch."
        )
        ...
        print(
            "  🔒 RESTRICTED by system policy (MDM/SIP). Cannot override from user space."
        )
    else:
