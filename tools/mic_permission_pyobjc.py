...
def get_status():
    ...
    raw = AVFoundation.AVCaptureDevice.authorizationStatusForMediaType_(
        AVFoundation.AVMediaTypeAudio)
    return {0: 'notDetermined', 1: 'restricted', 2: 'denied', 3: 'authorized'}.get(raw, f'unknown({raw})')
...
def request_permission_blocking(timeout_seconds=30):
    ...
    if current_status == 'denied':
        ...
        print("❌ Microphone denied. Open: System Settings → Privacy & Security → Microphone")
        return False
    ...
    def handler(granted):
        ...
        push_redis(f"mic_permission_pyobjc.py: handler fired granted={granted} status={new_status}")
        print(f"Handler: granted={granted}, status={new_status}")
        result['event'].set()

    push_redis("mic_permission_pyobjc.py: calling requestAccessForMediaType_completionHandler_")
    ...
    while not result['event'].is_set():
        ...
        if NSDate.date().compare_(deadline) != -1:  # NSOrderedDescending = 1, NSOrderedAscending = -1
    ...
    push_redis(f"mic_permission_pyobjc.py: FINAL granted={granted} status={final_status}")
...
def check_status():
    ...
    if status_str == 'authorized':
        ...
        print("⚠️  Run 'open ~/brain/agentic-brain/tools/PyMicPermission.app' to request permission")
    elif status_str == 'denied':
        print("❌ Open System Settings → Privacy & Security → Microphone → enable Python/Terminal")
    return status_str
